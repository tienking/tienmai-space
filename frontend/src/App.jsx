// App.jsx — Main portfolio page (tienmai.space)
// Template-inspired design: dark hero, orange/coral accent, service-card skills,
// dark footer. Self-contained: all colors explicit, no index.css CSS var deps.
...
// ── Global styles (replaces index.css for the portfolio bundle) ───────────────
const GLOBAL_CSS = `
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  @media(min-width: 768px) { .p-hero-grid { grid-template-columns: 1.15fr 0.85fr; } }

  .p-about-grid {
    display: grid; grid-template-columns: 1fr; gap: 32px;
  }
  @media(min-width: 768px) { .p-about-grid { grid-template-columns: 2fr 1fr; } }
    ...
    <>
          ...
          {profile.skills[0]?.group !== undefined ? (
              ...
              {profile.skills.map((grp, gi) => (
                <div key={gi} style={{
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                    {(grp.items || []).map((s, si) => (
                      <span key={si} style={{ fontSize: 11, fontFamily: "'DM Mono', monospace", color: A, background: `${A}0d`, border: `1px solid ${A}22`, borderRadius: 6, padding: "3px 10px" }}>
                        {s}
                      </span>
                    ))}
                  </div>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              {profile.skills.map((s, i) => (
                <span key={i} style={{ fontSize: 12, fontFamily: "'DM Mono', monospace", color: A, background: `${A}0d`, border: `1px solid ${A}22`, borderRadius: 20, padding: "5px 14px", transition: "all .2s", cursor: "default" }}
                  onMouseEnter={e => { e.currentTarget.style.background = A; e.currentTarget.style.color = "#fff"; }}
        ...
        <Sect id="education" bg="#f8f8f8">
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {profile.educations.map((edu, i) => (
              <div key={i} style={{ background: "#fff", borderRadius: 14, padding: "20px 22px", border: "1px solid #e5e7eb", borderLeft: `4px solid ${A}`, display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 10, transition: "all .25s" }}
        ...
        <Sect id="projects" bg="#fff">
          ...
          <div className="p-proj-grid">
            {profile.projects.map((p, i) => (
              <div key={i} style={{ background: "#fff", borderRadius: 16, border: "1px solid #e5e7eb", overflow: "hidden", transition: "all .25s", display: "flex", flexDirection: "column" }}
                ...
                <div style={{ padding: "20px 20px 22px", flex: 1, display: "flex", flexDirection: "column" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                    ...
                    {p.tag && <span style={{ fontSize: 10, fontFamily: "'DM Mono', monospace", color: A, background: `${A}10`, border: `1px solid ${A}28`, borderRadius: 6, padding: "2px 8px", flexShrink: 0 }}>{p.tag}</span>}
                  </div>
                  <p style={{ fontSize: 13, color: "#4b5563", lineHeight: 1.72, flex: 1 }}>{p.description}</p>
                  {p.link && <a href={p.link} target="_blank" rel="noreferrer" style={{ fontSize: 12, color: A, marginTop: 14, display: "inline-flex", alignItems: "center", gap: 4, textDecoration: "none", fontWeight: 600 }}>View project →</a>}
                </div>
              </div>
            ))}
          </div>
        ...
        <Sect id="gallery" bg="#fff">
          ...
          {galleryGroups.map(({ year, items }, gIdx) => (
            <div key={year ?? "noyear"} style={{ marginBottom: gIdx < galleryGroups.length - 1 ? 32 : 0 }}>
              {year && <p style={{ fontSize: 11, fontFamily: "'DM Mono', monospace", color: "#9ca3af", letterSpacing: ".12em", marginBottom: 12 }}>{year}</p>}
                ...
                {items.map(item => {
                  ...
                  return (
                    <div key={fi} onClick={() => setLbIdx(fi)} style={{ cursor: "pointer" }}>
                      ...
                      {cap && <p style={{ fontSize: 10, color: "#9ca3af", marginTop: 5, textAlign: "center", lineHeight: 1.4, overflow: "hidden", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" }}>{cap}</p>}
                    </div>
                  );
                })}
              </div>
      ...
      <footer style={{ background: BG_DARK, padding: "60px 24px 44px", textAlign: "center" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          ...
          {profile.title && <p style={{ fontSize: 13, color: "rgba(255,255,255,0.35)", fontFamily: "'DM Mono', monospace", marginBottom: 30 }}>{profile.title}</p>}
      ...
      {/* Chat popup: dark bg + orange accent via CSS var + color inheritance */}
      <div style={{
